...
```
需求收集 → 需求文档 + UI设计 → 前后端并行开发（含后端管理系统）+ 联调 → 测试 + 代码质量检查 → 部署到测试服务器 → 完成
```

| 阶段 | 输出 | 门禁 | 谁参与 |
|------|------|------|--------|
| 需求收集 | 需求讨论（逐个问题） | 核心功能确定 | PM + 用户 |
| 需求文档 + UI设计 | PRD文档 + UI设计方案（含后端管理系统） | 用户确认 | PM + UI Designer |
| 前后端并行开发 | 可运行代码（用户端 + 后端管理系统） | CI 测试全过 | Backend + Frontend + DevOps |
| 联调 | 前后端对接完成（含后端管理系统对接） | 联调通过 | Backend + Frontend |
...
**目标**：用户端和后端管理系统并行开发，互不依赖

**规则**：
- **环境自己搭建**：后端在本地或测试服务器搭建开发环境，前端在本地搭建
- **使用 Mock 数据**：前端使用 Mock 数据开发，不依赖后端接口
- **同步开发**：用户端和后端管理系统并行开发，互不阻塞
- **后端管理系统必须开发**：每个产品都要有配套的后端管理系统，方便运维
- **环境搭建规则**：
  - 本地能搞就在本地搞
...
- **2026-04-03**：后端管理系统作为产品标配，与用户端并行开发
